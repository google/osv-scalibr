...
func (Scanner) Scan(ctx context.Context, cfg *ScanConfig) (sr *ScanResult) {
	...
	// If the configuration of the plugin is not provided, we use the default one.
	// We store any closers (for resources created by the default config) in
	// internallyCreatedCloser to be closed at the end of the scan.
