...
var (
	...
	// It is initialized in init() from structToProtoAnnotations.
	protoToStructAnnotations = map[spb.Package_AnnotationEnum]extractor.Annotation{}
)
...
func packageToStruct(pkgProto *spb.Package) *extractor.Package {
	...
	for _, loc := range pkgProto.GetLocations() {
...
func init() {
	for k, v := range structToProtoAnnotations {
		protoToStructAnnotations[v] = k
	}
	if len(protoToStructAnnotations) != len(structToProtoAnnotations) {
