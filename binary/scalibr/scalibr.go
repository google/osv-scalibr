...
func parseFlags(args []string) (*cli.Flags, error) {
	...
	imageLocal := fs.String("image-local", "", "The image that is available in the local filesystem. These are the images from the output of `docker image ls`. If specified, SCALIBR scans this image. The name of the image MUST also include the tag of the image <image_name>:<image_tag>.")
