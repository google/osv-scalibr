...
func TestExtractorNamesUnique(t *testing.T) {
	all, err := pl.All(config.DefaultPluginConfig())
	if err != nil {
...
func TestFromCapabilities(t *testing.T) {
	...
	plugins, err := pl.FromCapabilities(capab, config.DefaultPluginConfig())
	if err != nil {
...
func TestFromNames(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			got, err := pl.FromNames(tc.names, config.DefaultPluginConfig())
...
func TestFromName(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			got, err := pl.FromName(tc.name, config.DefaultPluginConfig())
			if diff := cmp.Diff(tc.wantErr, err, cmpopts.EquateErrors()); diff != "" {
