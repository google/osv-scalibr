...
func (e *Enricher) Enrich(ctx context.Context, input *enricher.ScanInput, inv *inventory.Inventory) error {
	if !e.client.GoToolchainAvailable(ctx) {
	...
	var vulns []*vulnpb.Vulnerability
	...
	for goModLocation, goVersion := range goModVersions {
		...
		findings, err := e.client.RunGovulncheck(ctx, absModDir, vulns, goVersion)
		if err != nil {
...
func vulnHasImportsField(vuln *vulnpb.Vulnerability, pkg *extractor.Package) bool {
...
func NewWithClient(cfg *cpb.PluginConfig, client GovulncheckClient) enricher.Enricher {
	return &Enricher{
		client: client,
	}
}
