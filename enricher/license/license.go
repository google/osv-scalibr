...
func New(cfg *config.PluginConfig) (enricher.Enricher, error) {
	return &Enricher{Config: cfg}, nil
}
...
func (e *Enricher) Enrich(ctx context.Context, _ *enricher.ScanInput, inv *inventory.Inventory) error {
	if e.Client == nil {
		if e.Config == nil || e.Config.ClientFactories == nil {
			return fmt.Errorf("client factories not configured for %s", Name)
		}
		userAgent := "osv-scalibr/" + scalibrversion.ScannerVersion
		conn, err := e.Config.ClientFactories.GRPCClientConn(depsdev.DepsdevAPI, grpc.WithUserAgent(userAgent))
		if err != nil {
			return fmt.Errorf("failed to get gRPC connection for %s: %w", Name, err)
		}
		if conn == nil {
			return fmt.Errorf("gRPC connection is nil for %s", Name)
		}
		e.Client = datasource.NewCachedInsightsClientWithConn(conn)
	}
