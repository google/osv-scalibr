...
func TestEnrichersFromName(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			got, err := el.EnrichersFromName(tc.name, config.DefaultPluginConfig())
...
func TestEnrichersFromNameNilConfig(t *testing.T) {
	_, err := el.EnrichersFromName("layerdetails", nil)
	if err != nil {
		t.Errorf("EnrichersFromName(\"layerdetails\", nil) failed: %v", err)
	}
}

func TestEnricherFromNameNilConfig(t *testing.T) {
	_, err := el.EnricherFromName("license/depsdev", nil)
	if err != nil {
		t.Errorf("EnricherFromName(\"license/depsdev\", nil) failed: %v", err)
	}
}
