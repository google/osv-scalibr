...
func New(cfg Config) *Extractor {
	// Creates a new containerd API client using the provided socket address.
	cli, err := containerd.New(cfg.ContainerdSocketAddr)
	if err != nil {
		cli = nil
		log.Errorf("Failed to connect to containerd socket %v, error: %v", cfg.ContainerdSocketAddr, err)
	}
...
func (e Extractor) Requirements() *plugin.Requirements { return &plugin.Requirements{RealFS: true} }
