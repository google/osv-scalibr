...
import (
	...
	metadata "github.com/google/osv-scalibr/extractor/filesystem/misc/vscodeextensions"
...
var (
	metadataStruct1 = &metadata.Metadata{
		...
		TargetPlatform:       "test-target-platform",
		Updated:              true,
		IsPreReleaseVersion:  true,
		InstalledTimestamp:   1234567890,
	}
	metadataProto1 = &pb.VSCodeExtensionsMetadata{
		Id:                   "test-id",
		PublisherId:          "test-publisher-id",
		PublisherDisplayName: "Test Publisher Display Name",
		TargetPlatform:       "test-target-platform",
...
func TestSetProto(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			want: &pb.Package{
				Name: "some-package",
				Metadata: &pb.Package_VscodeExtensionsMetadata{
					VscodeExtensionsMetadata: metadataProto1,
				},
			},
		},
		{
			...
			m: &metadata.Metadata{
				ID:                   "overridden-id",
				PublisherID:          "overridden-publisher-id",
				PublisherDisplayName: "Overridden Publisher Display Name",
				TargetPlatform:       "overridden-target-platform",
				Updated:              false,
				IsPreReleaseVersion:  false,
				InstalledTimestamp:   987654321,
			},
			p: &pb.Package{
				Name: "some-package",
				Metadata: &pb.Package_VscodeExtensionsMetadata{
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			got := metadata.ToStruct(p.GetVscodeExtensionsMetadata())
			if diff := cmp.Diff(tc.m, got); diff != "" {
				t.Errorf("ToStruct(%+v): (-want +got):\n%s", p.GetVscodeExtensionsMetadata(), diff)
			}
		})
	}
}
