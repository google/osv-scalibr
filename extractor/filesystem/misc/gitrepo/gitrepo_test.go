...
func TestDirExtractor_FileRequired(t *testing.T) {
	tests := []struct {
		name      string
		inputPath string
		want      bool
...
func TestFileExtractor_FileRequired(t *testing.T) {
	tests := []struct {
		name      string
		inputPath string
		want      bool
	}{
		{name: "git file", inputPath: "/foo/bar/.git", want: true},
		{name: "not git", inputPath: "/foo/bar/baz", want: false},
	}
...
`
		...
		{
			name: "Commit SHA resolution returns error but yields a non-empty SHA",
			path: ".git",
			setupSubtest: func(t *testing.T, dotGit string) {
				t.Helper()
				oldResolve := resolveCommitSHA
				t.Cleanup(func() { resolveCommitSHA = oldResolve })
				resolveCommitSHA = func(input *filesystem.ScanInput) (string, error) {
					return "non-empty-sha-on-error", os.ErrPermission
				}
			},
			wantPackages: []*extractor.Package{
				{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			extr, err := NewDir(&cpb.PluginConfig{})
			if err != nil {
...
func TestFileExtractor_Extract(t *testing.T) {
	...
	// Simulate main repo cache
	mainGitModules := filepath.Join(tempDir, ".git", "modules", "libX")
	if err := os.MkdirAll(mainGitModules, 0755); err != nil {
		t.Fatalf("Failed to create modules dir: %v", err)
	}

	configContent := `
[remote "origin"]
	url = https://github.com/google/libX.git
`
	if err := os.WriteFile(filepath.Join(mainGitModules, "config"), []byte(configContent), 0644); err != nil {
		t.Fatalf("Failed to write config: %v", err)
	}
	commitSHA := "abcdef1234567890abcdef1234567890abcdef12"
	if err := os.WriteFile(filepath.Join(mainGitModules, "HEAD"), []byte(commitSHA+"\n"), 0644); err != nil {
		t.Fatalf("Failed to write HEAD: %v", err)
	}

	// Simulate submodule directory
	submoduleDir := filepath.Join(tempDir, "libX")
	if err := os.MkdirAll(submoduleDir, 0755); err != nil {
		t.Fatalf("Failed to create submodule dir: %v", err)
	}

	// In reality, the redirect path is relative to the .git file location
	// libX/.git contains "gitdir: ../.git/modules/libX"
	if err := os.WriteFile(filepath.Join(submoduleDir, ".git"), []byte("gitdir: ../.git/modules/libX\n"), 0644); err != nil {
	...
	tests := []struct {
		name         string
		path         string
		setupSubtest func(t *testing.T, submoduleDir string)
		wantPackages []*extractor.Package
		wantErr      bool
	}{
		{
			...
			setupSubtest: func(t *testing.T, submoduleDir string) {
				t.Helper()
				if err := os.WriteFile(filepath.Join(submoduleDir, ".git"), []byte("gitdir: ../.git/modules/libX\n"), 0644); err != nil {
					t.Fatalf("Failed to write valid .git file: %v", err)
				}
			},
			wantPackages: []*extractor.Package{
				{
					...
					Version:  "",
					PURLType: purl.TypeGithub,
					Location: extractor.LocationFromPath("libX/.git"),
					SourceCode: &extractor.SourceCodeIdentifier{
						Repo:   "https://github.com/google/libX",
						Commit: commitSHA,
					},
				},
			},
		...
		{
			name: "Malformed .git file (missing gitdir)",
			path: "libX/.git",
			setupSubtest: func(t *testing.T, submoduleDir string) {
				t.Helper()
				if err := os.WriteFile(filepath.Join(submoduleDir, ".git"), []byte("not a gitdir pointer\n"), 0644); err != nil {
					t.Fatalf("Failed to overwrite .git file: %v", err)
				}
			},
			wantErr: true, // Should fail to open config at wrong path
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			extr, err := NewFile(&cpb.PluginConfig{})
			if err != nil {
				t.Fatalf("NewFile failed: %v", err)
			}

			if tt.setupSubtest != nil {
				tt.setupSubtest(t, submoduleDir)
			}
...
func BenchmarkFileExtractor_Extract(b *testing.B) {
	...
	configContent := `
[remote "origin"]
	url = https://github.com/google/libXa.git
`
	if err := os.WriteFile(filepath.Join(mainGitModules, "config"), []byte(configContent), 0644); err != nil {
		b.Fatalf("Failed to write config: %v", err)
	}
	if err := os.WriteFile(filepath.Join(mainGitModules, "HEAD"), []byte("abcdef1234567890abcdef1234567890abcdef12\n"), 0644); err != nil {
		b.Fatalf("Failed to write HEAD: %v", err)
	}

	submoduleDir := filepath.Join(tempDir, "libX")
	if err := os.MkdirAll(submoduleDir, 0755); err != nil {
		b.Fatalf("Failed to create submodule dir: %v", err)
	}
	if err := os.WriteFile(filepath.Join(submoduleDir, ".git"), []byte("gitdir: ../.git/modules/libX\n"), 0644); err != nil {
		b.Fatalf("Failed to write .git file: %v", err)
	}
