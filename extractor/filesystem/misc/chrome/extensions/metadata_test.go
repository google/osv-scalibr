...
var (
	metadataStruct1 = &metadata.Metadata{
		...
		HostPermissions:      []string{"host_permission_1", "host_permission_2"},
		ManifestVersion:      1,
		MinimumChromeVersion: "123.0.0.0",
...
func TestSetProto(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			want: &pb.Package{
				Name: "some-package",
				Metadata: &pb.Package_ChromeExtensionsMetadata{
					ChromeExtensionsMetadata: metadataProto1,
				},
			},
		},
		{
			...
			m: &metadata.Metadata{
				Name:                 "Overridden Extension",
				Description:          "Overridden Description",
				AuthorEmail:          "overridden@example.com",
				HostPermissions:      []string{"overridden_host_permission_1", "overridden_host_permission_2"},
				ManifestVersion:      2,
				MinimumChromeVersion: "456.0.0.0",
				Permissions:          []string{"overridden_permission_1", "overridden_permission_2"},
				UpdateURL:            "https://example.com/overridden_update",
			},
			p: &pb.Package{
				Name: "some-package",
				Metadata: &pb.Package_ChromeExtensionsMetadata{
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			got := metadata.ToStruct(p.GetChromeExtensionsMetadata())
			if diff := cmp.Diff(tc.m, got); diff != "" {
				t.Errorf("ToStruct(%+v): (-want +got):\n%s", p.GetChromeExtensionsMetadata(), diff)
			}
		})
	}
}
