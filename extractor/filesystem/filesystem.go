...
func errToExtractorStatus(extractors []Extractor, foundInv map[string]bool, errs map[string]map[string]error) []*plugin.Status {
	...
	for _, ex := range extractors {
		overallErrString, fileErrs := createFileErrorsForPlugin(errs[ex.Name()])
		var overallErr error
		if overallErrString != "" {
			overallErr = errors.New(overallErrString)
		}
...
func createFileErrorsForPlugin(errorMap map[string]error) (string, []*plugin.FileErrors) {
	if len(errorMap) == 0 {
		return "", nil
	}
	...
	return fmt.Sprintf("encountered %d error(s) while running plugin; check file-specific errors for details", len(fileErrors)), fileErrors
}
