...
func TestFileRequired(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			e, err := packagesconfig.New(&cpb.PluginConfig{MaxFileSizeBytes: tt.maxFileSizeBytes})
			if err != nil {
				t.Fatalf("New() unexpected error: %v", err)
			}
			e.(*packagesconfig.Extractor).Stats = collector
...
func TestExtract(t *testing.T) {
	tests := []extracttest.TestTableEntry{
		{
			...
			WantPackages: []*extractor.Package{
				...
				},
				{
					...
					Version:  "1.0.0",
					PURLType: purl.TypeNuget,
					Location: extractor.LocationFromPathAndLine("testdata/valid", 4),
				},
			},
		},
		{
			...
			WantPackages: []*extractor.Package{
				{
					...
					Version:  "1.0.0",
					PURLType: purl.TypeNuget,
					Location: extractor.LocationFromPathAndLine("testdata/noversion", 3),
				},
			},
		},
		{
			...
			WantPackages: []*extractor.Package{
				{
					...
					Version:  "1.0.0",
					PURLType: purl.TypeNuget,
					Location: extractor.LocationFromPathAndLine("testdata/nopackage", 3),
				},
			},
		},
		{
			Name: "empty packages.config file",
			InputConfig: extracttest.ScanInputMockConfig{
				Path: "testdata/empty",
			},
			WantErr: cmpopts.AnyError,
		...
		{
			Name: "duplicate packages.config file",
			InputConfig: extracttest.ScanInputMockConfig{
	...
	for _, tt := range tests {
		t.Run(tt.Name, func(t *testing.T) {
			collector := testcollector.New()
			e, err := packagesconfig.New(&cpb.PluginConfig{MaxFileSizeBytes: 100})
			if err != nil {
				t.Fatalf("New() unexpected error: %v", err)
			}
			e.(*packagesconfig.Extractor).Stats = collector
