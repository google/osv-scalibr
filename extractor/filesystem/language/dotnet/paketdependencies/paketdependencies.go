...
func (e Extractor) parseDependenciesFile(reader io.Reader, path string) ([]*extractor.Package, error) {
	...
	for scanner.Scan() {
		...
		if matches := reGitHubPackage.FindStringSubmatch(line); len(matches) >= 2 {
			...
			if hasColon {
				ref := stripInlineComment(matches[2])
				...
				spaceValue := stripInlineComment(matches[3])
...
func extractVersionFromConstraint(constraint string) string {
	constraint = stripInlineComment(constraint)
	...
	// No operator found, but might have trailing comments/options.
	// Extract only the first word (version).
