...
// Package nugetcpm extracts packages from NuGet Central Package Management files.
package nugetcpm
...
func New(cfg *cpb.PluginConfig) (filesystem.Extractor, error) {
	...
	specific := plugin.FindConfig(cfg, func(c *cpb.PluginSpecificConfig) *cpb.NugetCPMConfig { return c.GetNugetCpm() })
...
// FileRequired returns true if the specified file matches the NuGet CPM Directory.Packages.props or Directory.Build.props pattern.
func (e Extractor) FileRequired(api filesystem.FileAPI) bool {
	path := api.Path()
	base := filepath.Base(path)
	if base != "Directory.Packages.props" && base != "Directory.Build.props" {
		return false
	}
...
// Extract parses the Directory.Packages.props file to extract NuGet CPM package dependencies.
...
func (e Extractor) extractFromInput(input *filesystem.ScanInput) ([]*extractor.Package, error) {
	...
	if err := decoder.Decode(&proj); err != nil {
		log.Errorf("Error parsing Directory.Packages.props: %v", err)
		return nil, err
	}
