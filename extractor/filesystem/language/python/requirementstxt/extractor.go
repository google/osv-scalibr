...
func parseRequirementsTxt(input *filesystem.ScanInput, requiredAlready map[string]struct{}) ([]*extractor.Inventory, error) {
	...
	for scanner.Scan() {
		...
		// Metadata will always be osv.DepGroupMetadata, as that is what we construct at the
		// start of this file
		existingGroups := d.Metadata.(osv.DepGroups).DepGroups()
		if !hasGroup(existingGroups) {
			d.Metadata = osv.DepGroupMetadata{
