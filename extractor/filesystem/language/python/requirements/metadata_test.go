...
import (
	...
	metadata "github.com/google/osv-scalibr/extractor/filesystem/language/python/requirements"
...
var (
	metadataStruct1 = &metadata.Metadata{
		HashCheckingModeValues: []string{"hash-checking-mode-value"},
		VersionComparator:      "version-comparator",
		Requirement:            "requirement",
	}
	metadataProto1 = &pb.PythonRequirementsMetadata{
		HashCheckingModeValues: []string{"hash-checking-mode-value"},
		VersionComparator:      "version-comparator",
		Requirement:            "requirement",
	}
...
func TestSetProto(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			want: &pb.Package{
				Name: "some-package",
				Metadata: &pb.Package_PythonRequirementsMetadata{
					PythonRequirementsMetadata: metadataProto1,
				},
			},
		},
		{
			...
			m: &metadata.Metadata{
				HashCheckingModeValues: []string{"other-hash-checking-mode-value"},
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			got := metadata.ToStruct(p.GetPythonRequirementsMetadata())
			if diff := cmp.Diff(tc.m, got); diff != "" {
				t.Errorf("ToStruct(%+v): (-want +got):\n%s", p.GetPythonRequirementsMetadata(), diff)
			}
		})
	}
}
...
func TestToStruct(t *testing.T) {
	testCases := []struct {
		desc string
		m    *pb.PythonRequirementsMetadata
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			wantP := &pb.Package{
				Metadata: &pb.Package_PythonRequirementsMetadata{
					PythonRequirementsMetadata: tc.m,
				},
			}
			got.SetProto(gotP)
			opts := []cmp.Option{
