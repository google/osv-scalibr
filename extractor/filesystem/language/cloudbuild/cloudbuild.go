...
// Package cloudbuild extracts container image references from Google Cloud Build
// configuration files (cloudbuild.yaml / cloudbuild.yml).
package cloudbuild
...
// cloudbuildConfig represents the top-level structure of a cloudbuild.yaml file.
...
// Extractor extracts container image references from Cloud Build configuration files.
type Extractor struct {
	Stats            stats.Collector
	maxFileSizeBytes int64
}

// New returns a Cloud Build image extractor.
//
...
// FileRequired returns true if the specified file is a Cloud Build configuration file.
func (e Extractor) FileRequired(api filesystem.FileAPI) bool {
	...
	if filename != "cloudbuild.yaml" && filename != "cloudbuild.yml" {
		return false
	}
...
// Extract extracts container image references from a Cloud Build configuration file.
func (e Extractor) Extract(ctx context.Context, input *filesystem.ScanInput) (inventory.Inventory, error) {
	...
	if err != nil {
		log.Debugf("cloudbuild: parse failed for %s: %v", input.Path, err)
		return inventory.Inventory{}, nil
	}
...
func parse(r io.Reader, path string) ([]*extractor.Package, error) {
	...
	for _, step := range cfg.Steps {
		...
		// Skip unresolved variable references (e.g. $PROJECT_ID, ${VAR})
