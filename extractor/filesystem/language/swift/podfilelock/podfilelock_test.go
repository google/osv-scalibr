...
func TestFileRequired(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			e, err := podfilelock.New(&cpb.PluginConfig{MaxFileSizeBytes: tt.maxFileSizeBytes})
			if err != nil {
				t.Fatalf("podfilelock.New: %v", err)
			}
			e.(*podfilelock.Extractor).Stats = collector
...
func TestExtract(t *testing.T) {
			...
			WantPackages: []*extractor.Package{
				{
					...
					Version:  "3.1.2",
					PURLType: purl.TypeCocoapods,
					Location: extractor.LocationFromPath("testdata/valid2"),
				},
				{
					Name:     "PINCache",
					Version:  "3.0.3",
	...
	for _, tt := range tests {
		t.Run(tt.Name, func(t *testing.T) {
			collector := testcollector.New()
			e, err := podfilelock.New(&cpb.PluginConfig{MaxFileSizeBytes: 100})
			if err != nil {
				t.Fatalf("podfilelock.New: %v", err)
			}
			e.(*podfilelock.Extractor).Stats = collector
