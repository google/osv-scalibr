...
// populateLineNumbers populates the Package location with the line number in the file where the
// package was defined.
//
		...
		case xml.StartElement:
			if element.Name.Local == tagComponent {
				...
				if group == "" || name == "" || version == "" {
					log.Warnf("Could not identify package key from empty attribute in component element: group %q, name %q, version %q", group, name, version)
					break
				}
