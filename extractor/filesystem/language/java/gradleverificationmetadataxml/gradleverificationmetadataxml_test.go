...
func TestExtractor_FileRequired(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			e, err := gradleverificationmetadataxml.New(&cpb.PluginConfig{})
			if err != nil {
				t.Fatalf("gradleverificationmetadataxml.New: %v", err)
			}
...
func TestExtractor_Extract(t *testing.T) {
	tests := []extracttest.TestTableEntry{
		...
		},
		{
			Name: "single quotes",
			InputConfig: extracttest.ScanInputMockConfig{
				Path: "testdata/single-quotes.xml",
			},
			WantPackages: []*extractor.Package{
				{
					Name:     "org.apache.pdfbox:pdfbox",
		...
		},
		{
			...
			InputConfig: extracttest.ScanInputMockConfig{
				Path: "testdata/attribute-order.xml",
			},
			WantPackages: []*extractor.Package{
				{
					Name:     "org.apache.pdfbox:pdfbox",
		...
		{
			// We expect the `group`, `name`, and `version` attributes to be in the Gradle
			// packages, as they are mandatory. However, the extractor will not fail to extract
			// package information; it will only miss the line number information.
	...
	for _, tt := range tests {
		t.Run(tt.Name, func(t *testing.T) {
			extr, err := gradleverificationmetadataxml.New(&cpb.PluginConfig{})
			if err != nil {
				t.Fatalf("gradleverificationmetadataxml.New: %v", err)
			}
