...
// Package kustomize extracts container image references from Kustomize
// configuration files (kustomization.yaml / kustomization.yml).
package kustomize
...
// kustomizationConfig represents the top-level structure of a kustomization.yaml file.
...
// Extractor extracts container image references from Kustomize configuration files.
type Extractor struct {
	Stats            stats.Collector
	maxFileSizeBytes int64
}

// New returns a Kustomize image extractor.
//
...
// FileRequired returns true if the specified file is a Kustomize configuration file.
...
// Extract extracts container image references from a Kustomize configuration file.
func (e Extractor) Extract(ctx context.Context, input *filesystem.ScanInput) (inventory.Inventory, error) {
	...
	if err != nil {
		log.Debugf("kustomize: parse failed for %s: %v", input.Path, err)
		return inventory.Inventory{}, nil
	}
...
func parse(r io.Reader, path string) ([]*extractor.Package, error) {
	...
	for _, img := range cfg.Images {
		...
		if img.Digest != "" {
			version = strings.TrimSpace(img.Digest)
		} else if img.NewTag != "" {
			version = strings.TrimSpace(img.NewTag)
		}
