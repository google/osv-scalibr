...
// Package skaffold extracts container image references from Skaffold
// configuration files (skaffold.yaml / skaffold.yml).
package skaffold
...
// skaffoldConfig represents the top-level structure of a skaffold.yaml file.
...
// Extractor extracts container image references from Skaffold configuration files.
type Extractor struct {
	Stats            stats.Collector
	maxFileSizeBytes int64
}

// New returns a Skaffold image extractor.
//
...
// FileRequired returns true if the specified file is a Skaffold configuration file.
func (e Extractor) FileRequired(api filesystem.FileAPI) bool {
	...
	if filename != "skaffold.yaml" && filename != "skaffold.yml" {
		return false
	}
...
// Extract extracts container image references from a Skaffold configuration file.
func (e Extractor) Extract(ctx context.Context, input *filesystem.ScanInput) (inventory.Inventory, error) {
	...
	if err != nil {
		log.Debugf("skaffold: parse failed for %s: %v", input.Path, err)
		return inventory.Inventory{}, nil
	}
...
func parse(r io.Reader, path string) ([]*extractor.Package, error) {
	...
	for _, artifact := range cfg.Build.Artifacts {
		name := strings.TrimSpace(artifact.Image)
		if name == "" {
			continue
		}
