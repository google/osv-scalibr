...
func (e *Extractor) Extract(ctx context.Context, input *filesystem.ScanInput) (inventory.Inventory, error) {
	...
	var diskImages []*inventory.DiskImage
	for i, p := range partitionList {
		partitionIndex := i + 1 // go-diskfs uses 1-based indexing
		getDiskFS := func(ctx context.Context) (scalibrfs.FS, error) {
		...
		diskImages = append(diskImages, &inventory.DiskImage{
			Path:      fmt.Sprintf("%s:%d", vmdkPath, partitionIndex),
			GetDiskFS: getDiskFS,
		})
	}
	return inventory.Inventory{DiskImages: diskImages}, nil
}
