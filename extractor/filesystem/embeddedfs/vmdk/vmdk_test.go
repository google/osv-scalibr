...
func TestExtractValidVMDK(t *testing.T) {
	...
	if len(inv.DiskImages) == 0 {
		t.Fatal("Extract returned no DiskImages")
	}

	for i, diskImage := range inv.DiskImages {
		t.Run(fmt.Sprintf("DiskImage_%d", i), func(t *testing.T) {
			if !strings.HasPrefix(diskImage.Path, path) {
				t.Errorf("DiskImage.Path = %q, want prefix %q", diskImage.Path, path)
			}

			fs, _ := diskImage.GetDiskFS(ctx)
