...
func TestExtractValidStaticVDI(t *testing.T) {
	extractor := vdi.New()
	path := "testdata/valid-ext-exfat-fat32-ntfs-static.vdi"
...
func TestExtractValidDynamicVDI(t *testing.T) {
	extractor := vdi.New()
	path := "testdata/valid-ext-exfat-fat32-ntfs-dynamic.vdi"
	info, err := os.Stat(path)
	if err != nil {
		t.Fatalf("os.Stat(%q) failed: %v", path, err)
	}
