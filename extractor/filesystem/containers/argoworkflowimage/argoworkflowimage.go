...
type argoTemplate struct {
	...
	// Steps and DAG templates can contain references to other templates,
	// but the images are defined in the templates themselves.
...
// Scripts also have an image field.
...
func parseArgoWorkflowYAML(ctx context.Context, r io.Reader) ([]string, error) {
	...
	for {
		...
		if doc.Kind != "Workflow" && doc.Kind != "WorkflowTemplate" && doc.Kind != "CronWorkflow" && doc.Kind != "ClusterWorkflowTemplate" {
			continue
		}
...
func extractImagesFromArgoWorkflow(doc *argoWorkflow) []string {
	...
	for _, tmpl := range doc.Spec.Templates {
