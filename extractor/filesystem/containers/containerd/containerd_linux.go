...
func (e Extractor) Extract(ctx context.Context, input *filesystem.ScanInput) (inventory.Inventory, error) {
	...
	if err != nil {
		return inventory.Inventory{}, fmt.Errorf("Could not read the containerd metadb file: %v", err)
	}
	...
	if input.Path == linuxMetaDBPath {
		...
		if err != nil {
			return inventory.Inventory{}, fmt.Errorf("Could not collect snapshots metadata from DB: %v", err)
		}
	}
