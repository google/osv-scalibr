...
func (e Extractor) Extract(ctx context.Context, input *filesystem.ScanInput) ([]*extractor.Package, error) {
	...
	if input.Info != nil && input.Info.Size() > e.maxMetaDBFileSize {
		return pkgs, fmt.Errorf("Containerd metadb file %s is too large: %d", input.Path, input.Info.Size())
	}
	...
	if err != nil {
		return pkgs, fmt.Errorf("Could not read the containerd metadb file: %v", err)
	}
	...
	if input.Path == linuxMetaDBPath {
		...
		if err != nil {
			return pkgs, fmt.Errorf("Could not collect snapshots metadata from DB: %v", err)
		}
	}
