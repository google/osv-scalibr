...
func GitignoreMatch(gitignores []GitignorePattern, path []string, isDir bool) bool {
	for _, p := range gitignores {
		if p != nil && p.Match(path, isDir) {
			return true
		}
	}
	return false
...
// specified directory.
func ParseDirForGitignore(fs scalibrfs.FS, path string) (GitignorePattern, error) {
	path = strings.TrimSuffix(path, "/")
	f, err := fs.Open(path + "/.gitignore")
	if err != nil {
...
func ParseParentGitignores(fs scalibrfs.FS, dirPath string) ([]GitignorePattern, error) {
	...
	for _, dir := range strings.Split(dirPath, "/") {
		path += dir + "/"
		gitignores, err := ParseDirForGitignore(fs, path)
		if err != nil {
			return nil, err
		}
