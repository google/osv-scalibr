...
var (
	...
	metadataProto1 = &pb.HomebrewPackageMetadata{
		Url:     "url",
		Head:    "head",
		Mirrors: []string{"mirror1", "mirror2"},
	}
...
func TestSetProto(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			want: &pb.Package{
				Name: "some-package",
				Metadata: &pb.Package_HomebrewMetadata{
					HomebrewMetadata: metadataProto1,
				},
			},
		},
		{
			...
			m: &metadata.Metadata{
				URL:     "another-url",
				Head:    "another-head",
				Mirrors: []string{"another-mirror1", "another-mirror2"},
			},
			p: &pb.Package{
				Name: "some-package",
				Metadata: &pb.Package_HomebrewMetadata{
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			got := metadata.ToStruct(p.GetHomebrewMetadata())
			if diff := cmp.Diff(tc.m, got); diff != "" {
				t.Errorf("ToStruct(%+v): (-want +got):\n%s", p.GetHomebrewMetadata(), diff)
			}
		})
	}
}
