...
// BrewPackage struct holds homebrew package information from homebrew package path.
...
func (e Extractor) Extract(ctx context.Context, input *filesystem.ScanInput) (inventory.Inventory, error) {
	...
	if p.AppFormulaPath != "" {
		pkg.Metadata = parseFormula(input.FS, p.AppFormulaPath)
	}
...
func SplitPath(origPath string) *BrewPackage {
	...
	for i, pathPart := range pathParts {
		...
		if (pathPart == cellarPath || pathPart == caskPath) && len(pathParts) > (i+3) {
			...
			if pathPart == cellarPath {
				...
				formulaFilePath := path.Join(strings.Join(origPathParts[:i+3], "/"), ".brew", brewPackage.AppName+".rb")
...
func parseFormula(fs fs.FS, formulaFilePath string) *metadata.Metadata {
	metadata := &metadata.Metadata{}
	file, err := fs.Open(formulaFilePath)
	if err != nil {
		return metadata
	}
	defer file.Close()
