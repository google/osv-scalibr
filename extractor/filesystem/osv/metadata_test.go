...
func TestSetProto(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			want: &pb.Package{
				Name: "some-package",
				Metadata: &pb.Package_OsvMetadata{
					OsvMetadata: metadataProto1,
				},
			},
		},
		{
			...
			m: &metadata.Metadata{
				PURLType:  "other-purl-type",
				Commit:    "other-commit",
				Ecosystem: "other-ecosystem",
				CompareAs: "other-compare-as",
			},
			p: &pb.Package{
				Name: "some-package",
				Metadata: &pb.Package_OsvMetadata{
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			got := metadata.ToStruct(p.GetOsvMetadata())
			if diff := cmp.Diff(tc.m, got); diff != "" {
				t.Errorf("ToStruct(%+v): (-want +got):\n%s", p.GetOsvMetadata(), diff)
			}
		})
	}
}
