...
func TestExtractorsFromName(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			name:     "python",
			wantExts: []string{"python/pdmlock", "python/pipfile", "python/pipfilelock", "python/poetrylock", "python/pylock", "python/condameta", "python/ipythoninstall", "python/uvlock", "python/wheelegg", "python/requirements", "python/setup", "python/pyprojecttoml"},
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			got, err := el.ExtractorsFromName(tc.name, configtest.NewFakePluginConfig())
