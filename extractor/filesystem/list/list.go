...
func FromCapabilities(capabs *plugin.Capabilities) []filesystem.Extractor {
	all := vals(All)
	exs := make([]filesystem.Extractor, 0, len(all))
	for _, initer := range all {
		exs = append(exs, initer())
	}
	return FilterByCapabilities(exs, capabs)
}
...
func ExtractorsFromNames(names []string) ([]filesystem.Extractor, error) {
	...
	for _, n := range names {
		if initers, ok := extractorNames[strings.ToLower(n)]; ok {
