...
var (
	...
	// Javascript build-time extractors.
	JavascriptBuild = InitMap{
		packagejson.Name: {packagejson.NewDefault},
	}
	...
	// Dotnet (.NET) build-time extractors.
	DotnetBuild = InitMap{
	...
	// BuildTime extractors find packages on built systems (e.g. parsing
	...
	Default = concat(
		JavaSource, JavaBuild,
		JavascriptSource, JavascriptBuild,
		PythonSource, PythonBuild,
		GoSource, GoBuild,
		OS,
	)
	...
	extractorNames = concat(All, InitMap{
		// Languages.
		"cpp":        vals(CppSource),
		"java":       vals(concat(JavaSource, JavaBuild)),
		"javascript": vals(concat(JavascriptSource, JavascriptBuild)),
		"python":     vals(concat(PythonSource, PythonBuild)),
		"go":         vals(concat(GoSource, GoBuild)),
		...
		"dotnet":     vals(concat(DotnetSource, DotnetBuild)),
		"php":        vals(PHPSource),
