...
func graphToInventory(g *resolve.Graph) (*inventory.Inventory, map[*extractor.Package][]resolve.NodeID) {
	...
	uniquePkgs := make(map[struct{ name, version string }]*extractor.Package)
	...
	for i, n := range g.Nodes[1:] {
		...
		var ok bool
		if pkg, ok = uniquePkgs[struct{ name, version string }{checkPkg.Name, checkPkg.Version}]; !ok {
			pkg = checkPkg
			uniquePkgs[struct{ name, version string }{pkg.Name, pkg.Version}] = pkg
