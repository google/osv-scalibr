...
func patchVulns(ctx context.Context, cl resolve.Client, vm matcher.VulnerabilityMatcher, resolved *remediation.ResolvedManifest, vulnIDs []string, opts *remediation.Options) (*remediation.ResolvedManifest, error) {
	resolved = &remediation.ResolvedManifest{
	...
	for {
		...
		var err error
		resolved.Graph, err = resolution.Resolve(ctx, cl, resolved.Manifest, opts.ResolveOptions)
		if err != nil {
			return nil, err
		}
