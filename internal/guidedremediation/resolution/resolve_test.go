...
func TestResolveNPM(t *testing.T) {
	...
	got, err := resolution.Resolve(context.Background(), cl, m, resolution.ResolveOptions{})
	if err != nil {
		t.Fatal(err)
	}
	_ = got.Canon()
...
func TestResolveMaven(t *testing.T) {
	...
	got, err := resolution.Resolve(context.Background(), cl, m, resolution.ResolveOptions{MavenManagement: true})
	if err != nil {
		t.Fatal(err)
	}
	_ = got.Canon()
