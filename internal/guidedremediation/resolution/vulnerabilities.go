...
func FindVulnerabilities(ctx context.Context, cl matcher.VulnerabilityMatcher, m manifest.Manifest, graph *resolve.Graph) ([]Vulnerability, error) {
	nodeVulns, err := cl.MatchVulnerabilities(ctx, graphToPackages(graph))
	if err != nil {
		return nil, err
	}
...
// graphToPackages is a helper function to convert a Graph into an Package for use with VulnerabilityMatcher.
func graphToPackages(g *resolve.Graph) []*extractor.Package {
	...
	for i, n := range g.Nodes[1:] {
		pkgs[i] = vulns.VKToPackage(n.Version)
	}
