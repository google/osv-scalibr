...
const (
	...
	validSPOAuth2ClientURL = "adb-myworkspace.1233322.azuredatabricks.net"
)

func TestSPOAuth2ClientDetectorAcceptance(t *testing.T) {
	velestest.AcceptDetector(
		t,
		databricks.NewSPOAuth2ClientDetector(),
		validSPOAuth2ClientURL+"\n"+validClientSecret+"\n"+"client_id:"+validClientID,
		databricks.SPOAuth2ClientCredentials{URL: validSPOAuth2ClientURL, Secret: validClientSecret, ID: validClientID},
	)
}

func TestSPOAuth2ClientDetector_Detect(t *testing.T) {
	engine, err := veles.NewDetectionEngine([]veles.Detector{databricks.NewSPOAuth2ClientDetector()})
	if err != nil {
		t.Fatal(err)
	}
...
`,
			want: []veles.Secret{
				databricks.SPOAuth2ClientCredentials{
