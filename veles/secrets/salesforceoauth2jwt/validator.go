...
func parsePrivateKey(input string) (crypto.PrivateKey, error) {
	...
	block, _ := pem.Decode(inputBytes)
	if block != nil {
		inputBytes = block.Bytes
		pemParsed = true
	}

	if key, err := x509.ParsePKCS8PrivateKey(inputBytes); err == nil {
		return key, nil
	}
	if key, err := x509.ParsePKCS1PrivateKey(inputBytes); err == nil {
		return key, nil
	}
