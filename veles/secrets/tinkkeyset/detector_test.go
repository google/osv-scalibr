...
func TestDetector_truePositives(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			want: []veles.Secret{tinkkeyset.TinkKeySet{
				Keys: `{"primaryKeyId":1976038263, "key":[{"keyData":{"typeUrl":"type.googleapis.com/google.crypto.tink.AesGcmKey", "value":"GhBi7O0TErBM9eTl3UppUGZg", "keyMaterialType":"SYMMETRIC"}, "status":"ENABLED", "keyId":1976038263, "outputPrefixType":"TINK"}]}`,
			}},
		},
		{
			name: "pretty json",
		...
		},
		{
			...
			want: []veles.Secret{tinkkeyset.TinkKeySet{
				Keys: `{"primaryKeyId":65177451,"key":[{"keyData":{"typeUrl":"type.googleapis.com/google.crypto.tink.AesGcmKey","value":"GhBcKD0QJVkaSbbFnrmuIV+e","keyMaterialType":"SYMMETRIC"},"status":"ENABLED","keyId":65177451,"outputPrefixType":"TINK"},{"keyData":{"typeUrl":"type.googleapis.com/google.crypto.tink.AesGcmKey","value":"GiCPZRivnJJeqCAXGJemTND9SmOMQkCwxbHGUXGoRmvZcw==","keyMaterialType":"SYMMETRIC"},"status":"ENABLED","keyId":3858149861,"outputPrefixType":"TINK"}]}`,
			}},
		},
		{
			...
			want: []veles.Secret{tinkkeyset.TinkKeySet{
				Keys: `{"primaryKeyId":1045837809,"key":[{"keyData":{"typeUrl":"type.googleapis.com/google.crypto.tink.AesGcmKey","value":"GhBC+B6GqV6rZI4jIA8XXDVD","keyMaterialType":"SYMMETRIC"},"status":"ENABLED","keyId":1045837809,"outputPrefixType":"TINK"}]}`,
			}},
		},
		{
	...
	cmpOpt := cmp.Comparer(func(x, y tinkkeyset.TinkKeySet) bool {
		...
		if err := json.Unmarshal([]byte(y.Keys), &yJSON); err != nil {
			return false
		}
