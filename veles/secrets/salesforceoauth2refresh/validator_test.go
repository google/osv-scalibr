...
const validatorTestRefresh = "123456789ABCDEFABC1234567895123459876ABCDEFABC1234567895"
...
func mockSalesforceServer(t *testing.T, expectedRefreshToken string, expectedBase64Data string, serverResponseCode int) *httptest.Server {
	t.Helper()
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if len(expectedBase64Data) > 0 && !strings.Contains(authHeader, expectedRefreshToken) && !strings.Contains(authHeader, expectedBase64Data) {
...
func TestValidator(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			server := mockSalesforceServer(t, validatorTestRefresh, tc.serverExpectedBase64Data, tc.serverResponseCode)
			defer server.Close()
			...
			validator := salesforceoauth2refresh.NewValidator()
			validator.HTTPC = client

			// Create test credentials
			cred := salesforceoauth2refresh.Credentials{ID: tc.id, Secret: tc.secret, Refresh: tc.refresh}
...
func TestValidator_ContextCancellation(t *testing.T) {
	...
	// Create test credentials
	cred := salesforceoauth2refresh.Credentials{ID: validatorTestClientID, Secret: validatorTestClientSecret, Refresh: validatorTestRefresh}
