...
const (
	// Tight window to pair env-style credentials across nearby lines.
	maxPairWindowLen = 256
	// Max length of a JWT token (delegated to common jwt limits).
...
func findMatches(re *regexp.Regexp, data []byte) []match {
	...
	for _, m := range idxs {
		...
		out = append(out, match{start: m[0], value: string(data[m[2]:m[3]])})
	}
	return out
}
...
func pairWithinWindow(ids, secs []match, window int) (pairs [][2]match, leftoverIDs, leftoverSecs []match) {
	...
	for i := range ids {
		...
		if j < len(secs) && secs[j].start <= id.start+window {
			pairs = append(pairs, [2]match{id, secs[j]})
			usedSec[j] = true
			j++
			continue
		}
