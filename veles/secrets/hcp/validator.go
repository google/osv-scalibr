...
const (
	defaultTokenURL = "https://auth.hashicorp.com/oauth/token"
...
func (v *ClientCredentialsValidator) Validate(ctx context.Context, cc ClientCredentials) (veles.ValidationStatus, error) {
	...
	if res.StatusCode == http.StatusOK {
		// Optionally verify body has an access_token field.
		var body struct {
			AccessToken string `json:"access_token"`
		}
		_ = json.NewDecoder(res.Body).Decode(&body)
		if body.AccessToken == "" {
			// Treat as valid since endpoint accepted creds, even if body parsing failed.
