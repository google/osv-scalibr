...
// Package onepasswordconnecttoken extracts OnePassword Connect Token secrets from JSON files.
...
func (e Extractor) FileRequired(api filesystem.FileAPI) bool {
	...
	return strings.Contains(fileName, "onepassword") || strings.Contains(fileName, "1password")
}
...
func (e Extractor) Extract(ctx context.Context, input *filesystem.ScanInput) (inventory.Inventory, error) {
	...
	if err := decoder.Decode(&data); err != nil {
		return inventory.Inventory{}, fmt.Errorf("failed to parse JSON: %w", err)
	}
	...
	pkg := &extractor.Package{
		...
		Metadata: &onepasswordconnecttokenmeta.Metadata{
