...
func TestSetProto(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			want: &pb.Package{
				Name: "some-package",
				Metadata: &pb.Package_OnePasswordConnectTokenMetadata{
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			got := metadata.ToStruct(p.GetOnePasswordConnectTokenMetadata())
			if diff := cmp.Diff(tc.m, got); diff != "" {
				t.Errorf("ToStruct(%+v): (-want +got):\n%s", p.GetOnePasswordConnectTokenMetadata(), diff)
			}
		})
	}
}
...
func TestToStruct(t *testing.T) {
	testCases := []struct {
		desc string
		m    *pb.OnePasswordConnectTokenMetadata
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			wantP := &pb.Package{
				Metadata: &pb.Package_OnePasswordConnectTokenMetadata{
					OnePasswordConnectTokenMetadata: tc.m,
				},
			}
			got.SetProto(gotP)
			opts := []cmp.Option{
