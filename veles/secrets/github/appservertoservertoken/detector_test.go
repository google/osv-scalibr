...
const (
	testKey        = `gh` + `s_oJrI3NxJonXega4cd3v1XHDjjMk3jh2ENWzb`
	anotherTestKey = `gh` + `s_DCASJRv332FYglZzXPw7n0onKEqqt50ugSfn`
)

// TestDetector_truePositives tests for cases where we know the Detector
// will find a Github app server to server tokens.
func TestDetector_truePositives(t *testing.T) {
	engine, err := veles.NewDetectionEngine([]veles.Detector{appservertoservertoken.NewDetector()})
	if err != nil {
		t.Fatal(err)
	}
	cases := []struct {
		...
		input: testKey,
		want: []veles.Secret{
			appservertoservertoken.GithubAppServerToServerToken{Token: testKey},
		},
	}, {
		name:  "simple matching string another key",
		input: anotherTestKey,
		want: []veles.Secret{
			appservertoservertoken.GithubAppServerToServerToken{Token: anotherTestKey},
		},
	}, {
...
func TestDetector_trueNegatives(t *testing.T) {
	...
	cases := []struct {
		...
		input: `gh` + `s_oJrI3NxJonXeg-4cd3v1XHDjjMk3jh2ENWzb`,
	}, {
		...
		input: `gh` + `s_oJrI3NxJonXega4cd2v1XHDjjMk3jh2ENWzb`,
	}, {
		name:  "prefix missing dash should not match",
		input: `gh` + `soJrI3NxJonXega4cd2v1XHDjjMk3jh2ENWzb`,
	}}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := engine.Detect(t.Context(), strings.NewReader(tc.input))
