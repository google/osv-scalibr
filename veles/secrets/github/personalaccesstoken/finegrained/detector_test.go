...
import (
	...
	"github.com/google/osv-scalibr/veles/secrets/github/personalaccesstoken/finegrained"
)

// Note: Github tokens are encoded using `` + `` to bypass github security checks

const (
	testKey        = `github` + `_pat_11ALJFEII0ZiQ19DEeBWSe_apMVlTnpi9UgqDHLAkMLh7iVx63tio9DckV9Rjqas6H4K5W45OQZK6Suog5`
	anotherTestKey = `github` + `_pat_11ALJFEII0UlnAoY24TCtP_haWQRFX8YZ4vniyajJ3GVbZ5VgNrrEyWFBq3VXgQzQO2M4XQFJMImiHXm6q`
)

// TestDetector_truePositives tests for cases where we know the Detector
// will find a Github app refresh tokens.
func TestDetector_truePositives(t *testing.T) {
	engine, err := veles.NewDetectionEngine([]veles.Detector{finegrained.NewDetector()})
	if err != nil {
		t.Fatal(err)
	}
	cases := []struct {
		...
		input: testKey,
		want: []veles.Secret{
			personalaccesstoken.GithubPersonalAccessToken{Token: testKey},
		},
	}, {
		name:  "simple matching string another key",
		input: anotherTestKey,
		want: []veles.Secret{
			personalaccesstoken.GithubPersonalAccessToken{Token: anotherTestKey},
		},
	}, {
...
func TestDetector_trueNegatives(t *testing.T) {
	...
	cases := []struct {
		...
		input: `github` + `_pat_11ALJFEII0Zi+19DEeBWSe_apMVlTnpi9UgqDHLAkMLh7iVx63tio9DckV9Rjqas6H4K5W45OQZK6Suog5`,
	}, {
		...
		input: `github` + `pat11ALJFEII0ZiQ19DEeBWSe_apMVlTnpi9UgqDHLAkMLh7iVx63tio9DckV9Rjqas6H4K5W45OQZK6Suog5`,
	}}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := engine.Detect(t.Context(), strings.NewReader(tc.input))
