...
import (
	...
	"github.com/google/osv-scalibr/veles/secrets/github/personalaccesstoken/classic"
)

// Note: Github tokens are encoded using `` + `` to bypass github security checks

const (
	testKey        = `gh` + `p_lbSH4CWqHKWSJCtf6JdQKnIkM6IkV00NzVax`
	anotherTestKey = `gh` + `p_HqVdKoLwkXN58VKftd2vJr0rxEx6tt26hion`
)

// TestDetector_truePositives tests for cases where we know the Detector
// will find a Github app refresh tokens.
func TestDetector_truePositives(t *testing.T) {
	engine, err := veles.NewDetectionEngine([]veles.Detector{classic.NewDetector()})
	if err != nil {
		t.Fatal(err)
	}
	cases := []struct {
