...
package personalaccesstoken_test
...
const (
	validatorFineGrainedKey = `github` + `_pat_11ALJFEII0ZiQ19DEeBWSe_apMVlTnpi9UgqDHLAkMLh7iVx63tio9DckV9Rjqas6H4K5W45OQZK6Suog5`
	validatorClassicKey     = `gh` + `p_HqVdKoLwkXN58VKftd2vJr0rxEx6tt26hion`
)
...
func mockGithubServer(t *testing.T, code int) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method != http.MethodGet || r.URL.Path != "/user" {
			t.Errorf("unexpected request: %s %s, expected: GET /user", r.Method, r.URL.Path)
		...
		if authHeader != "Bearer "+validatorFineGrainedKey && authHeader != "Bearer "+validatorClassicKey {
			w.Header().Set("Content-Type", "application/json")
...
func TestValidator(t *testing.T) {
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			validator := personalaccesstoken.NewValidator(
				personalaccesstoken.WithClient(client),
			)

			// Create a test key
			key := personalaccesstoken.GithubPersonalAccessToken{Token: tt.token}
