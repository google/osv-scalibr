...
package databricksuseraccountoauth2client
...
const (
	...
	// maxDistance is the maximum distance between User Account Client IDs, Secrects, and IDs to be considered for pairing.
...
var (
	// clientIDRe is a regular expression that matches Databricks User Account OAuth2 Client IDs.
	...
	// clientSecretRe is a regular expression that matches Databricks User Account OAuth2 Client Secrets.
...
// NewDetector returns a detector that matches Databricks User Account OAuth2 Client Credentials.
func NewDetector() veles.Detector {
	return &ntuple.Detector{
		...
		FromTuple: func(ms []ntuple.Match) (veles.Secret, bool) {
			return Credentials{ID: string(ms[0].Value), Secret: string(ms[1].Value), AccountID: string(ms[2].Value)}, true
		},
	}
}
