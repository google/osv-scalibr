...
// mockNpmRegistryServer creates a mock npm registry API server for testing
func mockNpmRegistryServer(t *testing.T, expectedKey string, serverResponseCode int) *httptest.Server {
	t.Helper()

	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Check if it's a GET request to the expected endpoint
		if r.Method != http.MethodGet || r.URL.Path != "/-/whoami" {
			t.Errorf("unexpected request: %s %s, expected: GET /-/whoami", r.Method, r.URL.Path)
...
func TestValidator(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			server := mockNpmRegistryServer(t, tc.serverExpectedKey, tc.serverResponseCode)
			defer server.Close()
