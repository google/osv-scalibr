...
const (
	validatorTestKey = "sk-a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6"
)

// mockQwenServer creates a mock DashScope API server for testing.
func mockQwenServer(t *testing.T, expectedKey string, statusCode int) *httptest.Server {
	t.Helper()

	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter,
		r *http.Request) {
		modelsEndpoint := qwenaiapikey.ModelsEndpoint
...
func TestValidator(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			server := mockQwenServer(t, validatorTestKey, tc.statusCode)
			defer server.Close()

			validator := qwenaiapikey.NewValidator()
			validator.HTTPC = server.Client()
			validator.Endpoint = server.URL + qwenaiapikey.ModelsEndpoint

			key := qwenaiapikey.APIKey{Key: validatorTestKey}
