...
// OrgReadToken is a Veles Secret that holds a Packagist Organization token with read-only access.
type OrgReadToken struct {
	Token   string
...
// ConductorUpdateToken is a Veles Secret that holds a Packagist Conductor token with update access.
type ConductorUpdateToken struct {
	Token string
}
