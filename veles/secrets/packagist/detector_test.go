...
// TestAPISecretDetector tests for cases where we expect the APISecretDetector
// to find or not find Packagist API Secrets (when paired with keys).
func TestAPISecretDetector(t *testing.T) {
	engine, err := veles.NewDetectionEngine([]veles.Detector{packagist.NewAPISecretDetector()})
	if err != nil {
		t.Fatal(err)
	}
...
// the detector creates an APISecret with the Key field populated.
...
// TestAPISecretDetector_SecretAndKey tests that when the APISecretDetector is used,
// we get APISecret when both key and secret are present together.
func TestAPISecretDetector_SecretAndKey(t *testing.T) {
