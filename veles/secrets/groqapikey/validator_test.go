...
const validatorTestKey = "gsk_0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOP"
...
// mockGroqServer creates a mock Groq API server for testing.
func mockGroqServer(t *testing.T, expectedKey string, statusCode int) *httptest.Server {
	t.Helper()
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method != http.MethodGet || r.URL.Path != "/openai/v1/models" {
			t.Errorf("unexpected request: %s %s, expected: GET /openai/v1/models", r.Method, r.URL.Path)
...
func TestValidator(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			// Create mock server.
			server := mockGroqServer(t, validatorTestKey, tc.statusCode)
			defer server.Close()
			...
			validator := groqapikey.NewValidator()
			validator.HTTPC = client

			key := groqapikey.GroqAPIKey{Key: validatorTestKey}
