...
func TestDetector_truePositives(t *testing.T) {
	engine, err := veles.NewDetectionEngine([]veles.Detector{pyxkeyv1.NewDetector()})
	if err != nil {
		t.Fatal(err)
	}
	cases := []struct {
		...
		want: []veles.Secret{
			pyxkeyv1.PyxKeyV1{Key: testKeyMixedCase},
		},
	}, {
		...
		want: []veles.Secret{
			pyxkeyv1.PyxKeyV1{Key: testKey},
			pyxkeyv1.PyxKeyV1{Key: testKey[:len(testKey)-1] + "1"},
		},
	}, {
...
// will not find a pyx v1 user key.
func TestDetector_trueNegatives(t *testing.T) {
	...
	cases := []struct {
		...
		input: `SK-pyx-testestestestestestestestestestestestestestestestestestestestest`,
	}, {
		name:  "special character in key should not match",
		input: `sk-pyx-testestestestestestestestestestestestestestestestestestestestes.`,
	}, {
		name:  "unexpected special character in prefix should not match",
		input: `sk.pyx-testestestestestestestestestestestestestestestestestestestestest`,
	}, {
		name:  "special character after prefix should not match",
		input: `sk-pyx-.estestestestestestestestestestestestestestestestestestestestest`,
	}}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := engine.Detect(t.Context(), strings.NewReader(tc.input))
