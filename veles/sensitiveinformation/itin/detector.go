...
var itinKeywords = simpleregex.KeywordsRe([]string{
	`\bitin\b`,
	`\bitin\s*(?:number|num|no|#)\b`,
	`individual\s*tin`,
	`\btin\b`,
	`\btin\s*(?:number|num|no|#)\b`,
	`individual\s*tax`,
	`individual\s*taxpayer`,
	`individual\s*taxpayer\s*id`,
	`individual\s*taxpayer\s*identification`,
	`taxpayer\s*identification\s*(?:number|num|no|#)?`,
	`tax\s*identification\s*(?:number|num|no|#)?`,
	`taxpayer\s*id`,
	`tax\s*id`,
	`irs\s*(?:individual\s*)?(?:taxpayer\s*)?(?:id|identification|tin|itin)`,
	`form\s*w-?7`,
	`\bw-?7\b`,
})
