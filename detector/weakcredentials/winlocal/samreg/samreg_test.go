...
func TestNewFromFile(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "File is not a registry file returns error",
			filepath:    "/dev/zero",
			wantErr:     true,
...
func TestUserRIDs(t *testing.T) {
	tests := []struct {
		...
		{
			name: "No users returns empty list",
			registry: &mockregistry.MockRegistry{
				Keys: map[string]registry.Key{
					`SAM\Domains\Account\Users`: &mockregistry.MockKey{
						KSubkeys: []registry.Key{
		...
		{
			name:     "Key containing user does not exist returns error",
			registry: &mockregistry.MockRegistry{},
...
func TestUserInfo(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Both V and F structure present returns no error",
			rid:  "000001F4",
			registry: &mockregistry.MockRegistry{
				Keys: map[string]registry.Key{
		...
		{
			name:        "User-specific key not found returns error",
			rid:         "000001F4",
			registry:    &mockregistry.MockRegistry{},
			wantErr:     true,
		...
		{
			name: "Missing V structure returns error",
			rid:  "000001F4",
			registry: &mockregistry.MockRegistry{
				Keys: map[string]registry.Key{
					`SAM\Domains\Account\Users\000001F4`: &mockregistry.MockKey{
		...
		{
			name: "Structure V is too short returns error",
			rid:  "000001F4",
			registry: &mockregistry.MockRegistry{
				Keys: map[string]registry.Key{
					`SAM\Domains\Account\Users\000001F4`: &mockregistry.MockKey{
		...
		{
			name: "Missing F structure returns error",
			rid:  "000001F4",
			registry: &mockregistry.MockRegistry{
				Keys: map[string]registry.Key{
					`SAM\Domains\Account\Users\000001F4`: &mockregistry.MockKey{
...
func TestDeriveSyskey(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Derived key is returned correctly",
			registry: &mockregistry.MockRegistry{
				Keys: map[string]registry.Key{
		...
		{
			name: "Registry key absent returns error",
			registry: &mockregistry.MockRegistry{
				Keys: map[string]registry.Key{},
			},
		...
		{
			name: "F domain value absent returns error",
			registry: &mockregistry.MockRegistry{
				Keys: map[string]registry.Key{
					`SAM\Domains\Account`: &mockregistry.MockKey{
						KValues: []registry.Value{},
		...
		{
			name: "Derivation error is propagated",
			registry: &mockregistry.MockRegistry{
				Keys: map[string]registry.Key{
					`SAM\Domains\Account`: &mockregistry.MockKey{
						KValues: []registry.Value{
