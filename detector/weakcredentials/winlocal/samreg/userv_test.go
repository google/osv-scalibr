...
func TestNewUserV(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "With valid buffer",
			buffer: []byte(strings.Repeat("\x00", 0xCC)),
		},
		{
			name:        "With invalid buffer",
			buffer:      []byte("A"),
			wantErr:     true,
...
func TestUserVUsername(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "Username offset is out of bounds",
