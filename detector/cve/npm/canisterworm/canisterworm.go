...
var (
	maliciousNpmPackages = map[string][]string{
	...
	maliciousPypiPackages = map[string][]string{
...
func (d Detector) Scan(ctx context.Context, scanRoot *fs.ScanRoot, px *packageindex.PackageIndex) (inventory.Finding, error) {
	...
	for _, pkg := range px.GetAllOfType(purl.TypeNPM) {
		if versions, ok := maliciousNpmPackages[pkg.Name]; ok {
			if slices.Contains(versions, pkg.Version) {
				f := d.findingForPackage(pkg.Name, pkg.Version, versions)
				result.GenericFindings = append(result.GenericFindings, f)
			}
		}
	}

	for _, pkg := range px.GetAllOfType(purl.TypePyPi) {
		if versions, ok := maliciousPypiPackages[pkg.Name]; ok {
			if slices.Contains(versions, pkg.Version) {
				f := d.findingForPackage(pkg.Name, pkg.Version, versions)
				result.GenericFindings = append(result.GenericFindings, f)
			}
...
func (d Detector) findingForPackage(pkgname, version string, unsafeVersions []string) *inventory.GenericFinding {
...
func (d Detector) vulnerability() *inventory.GenericFindingAdvisory {
	return &inventory.GenericFindingAdvisory{
		...
		Description:    "One or more packages were identified as potentially compromised",
