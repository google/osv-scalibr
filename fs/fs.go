...
type ScanRoot struct {
	...
	// scanning environment doesn't support the RealFS requirement.
	Path string
}
...
// RealFSScanRoot returns a one-element ScanRoot array representing the given
// root path on the real filesystem SCALIBR is running on.
func RealFSScanRoot(path string) []*ScanRoot {
	return []*ScanRoot{&ScanRoot{FS: DirFS(path), Path: path}}
}
