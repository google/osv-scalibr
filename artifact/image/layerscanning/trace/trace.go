...
func PopulateLayerDetails(ctx context.Context, inventory []*extractor.Inventory, chainLayers []scalibrImage.ChainLayer, config *filesystem.Config) {
	...
	for _, inv := range inventory {
		...
		for i := len(chainLayers) - 2; i >= 0; i-- {
			...
			} else if findFilesInLayer(oldChainLayer, inv.Locations) {
...
// findFilesInLayer checks if any of the provided files are present in the underlying layer of the
// chain layer.
func findFilesInLayer(chainLayer scalibrImage.ChainLayer, fileLocations []string) bool {
