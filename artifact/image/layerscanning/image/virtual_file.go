...
type virtualFile struct {
	// The reference to the content blob that the file contents are stored in. The content blob must
	// be opened before the virtual file is used.
	contentBlob *io.SectionReader
...
func (f *virtualFile) ReadAt(b []byte, off int64) (n int, err error) {
	...
	return f.contentBlob.ReadAt(b, off)
}
...
func (f *virtualFile) Seek(offset int64, whence int) (n int64, err error) {
	...
	return f.contentBlob.Seek(offset, whence)
}
...
func (f *virtualFile) Close() error {
	...
	_, err := f.contentBlob.Seek(0, io.SeekStart)
	return err
}
