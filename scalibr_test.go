...
func (e fakeExNeedsNetwork) Ecosystem(i *extractor.Inventory) (string, error)        { return "", nil }

func (fakeExNeedsNetwork) Requirements() *plugin.Requirements {
	return &plugin.Requirements{Network: true}
}
...
func (fakeDetNeedsFS) Scan(ctx context.Context, fs scalibrfs.FS, scanRoot string, ix *inventoryindex.InventoryIndex) ([]*detector.Finding, error) {
	return nil, nil
}
func (fakeDetNeedsFS) Requirements() *plugin.Requirements {
	return &plugin.Requirements{RealFS: true}
}
	...
	cases := []struct {
	...
	}{
		{
			...
			cfg: scalibr.ScanConfig{
				...
				Capabilities: &plugin.Requirements{
					Network: true,
					RealFS:  true,
				},
			},
