...
func CloseTestScanInput(tb testing.TB, si filesystem.ScanInput) {
	tb.Helper()
...
func GenerateScanInputMock(tb testing.TB, config ScanInputMockConfig) filesystem.ScanInput {
	tb.Helper()
	...
	if filepath.IsAbs(config.FakeScanRoot) {
		...
		if err != nil {
			tb.Fatalf("Can't get working directory because '%s'", workingDir)
		}
		scanRoot = filepath.Join(workingDir, config.FakeScanRoot)
	}

	fsys := os.DirFS(scanRoot)
	f, err := fsys.Open(filepath.ToSlash(config.Path))
	if err != nil {
		tb.Fatalf("Can't open test fixture '%s' because '%s'", config.Path, err)
	}
	info, err := f.Stat()
	if err != nil {
		tb.Fatalf("Can't stat test fixture '%s' because '%s'", config.Path, err)
	}

	return filesystem.ScanInput{
		FS:     fsys.(scalibrfs.FS),
		Path:   config.Path,
		Root:   scanRoot,
		Reader: f,
		Info:   info,
